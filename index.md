...
+ **Unit 4**: You should know how to formulate Lagrange's equation for systems with or without nonconservative forces. How to formulate the generalized force ${\mathbf{F}}{\scriptsize{nc}}^{*}$ (right hand side of Lagrange's equation). Also how to rewrite Lagrange's equation in the standard form $\mathbf{M}·\mathbf{\ddot{q}} + \mathbf{C}·\mathbf{\dot{q}} + \mathbf{G} = 
{\mathbf{F}}{\scriptsize{nc}}^{*}$ and how to convert this equation into a 1st order ODE (to have it prepared to solve initial value problems numerically).
